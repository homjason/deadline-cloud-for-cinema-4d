...
class TestFloatTileInternalSave:
    ...
    def test_float_tile_uses_internal_save_and_disables_bake(self, tmp_path):
        ...
        with patch.object(
            tile_rendering.tempfile,
            "mkdtemp",
            return_value=str(tmp_path / "c4dtile_7_0_0_x"),
        ):
            ctx = tile_rendering.setup_tile_render(render_data, self._data())

        import os

        # temp base lives in a private temp dir named with frame/col/row
        assert "c4dtile_7_0_0" in ctx.internal_save_base
        assert os.path.dirname(ctx.internal_save_base) == str(tmp_path / "c4dtile_7_0_0_x")
    ...
    def test_finalize_crops_from_internal_save_and_cleans_up(self, tmp_path):
        ...
        saved = tmp_path / "out_c4dtile_0_00000.exr"
        ...
        ctx = tile_rendering.TileContext(
            ...
            save_bits=4,
            internal_save_base=str(tmp_path / "out_c4dtile_0_0"),
        ...
        assert not saved.exists()  # temp full-frame file removed
    ...
    def test_finalize_restores_paths_when_locating_file_raises(self, tmp_path):
        ...
        ctx = tile_rendering.TileContext(
            ...
            requires_baking=False,
            save_bits=4,
            internal_save_base=str(tmp_path / "missing_c4dtile_0_0_0"),
